"""
Solution to the Assignment on Numerical Integration: Trapezoidal Rule
...
# trapezoid rule for calculating definite integral
...
def absolute_relative_error_finder(iteration):
    ...
    for i in range(iteration):
        values.append(str(approximate_vertical_distance(i + 1)))
        errors.append(str(round((math.fabs(
            approximate_vertical_distance(i + 2) - approximate_vertical_distance(i + 1)) / (
                                     approximate_vertical_distance(i + 2)) * 100), 4)))
    return values, errors


if __name__ == '__main__':
    iterations = int(input("Provide the highest number of sub-intervals: "))
    result = approximate_vertical_distance(iterations)
    print(result)
    ...
    table = pd.DataFrame(list(zip([i + 1 for i in range(iterations)], calculated_values, relative_errors)),
                         columns=['Number of Sub-Intervals', 'Approximate Vertical Distance(meter)',
